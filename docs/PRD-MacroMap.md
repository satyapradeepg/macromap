...
**Capacity dependency — reversed conclusion (July 2026, see OQ6):** this note previously said KR1's 1,000-subscriber target wasn't sized to run on the initial $29/month Spoonacular tier (~343 generations/week ceiling vs. ~600/week needed at KR2's rate). A recompute grounded directly in the shipped `web/` source (not the original theoretical model) found the real ceiling is **~938–1,876 generations/week**, comfortably above the ~600/week KR2 needs — the original ~49/day figure assumed 3 separate meal-type queries per plan, but the actual code collapsed this to 1 shared query since all 3 meal types target an identical per-meal split (see Section 7.4/OQ6 for the full derivation). **This is not yet a fully closed question:** the recompute is confirmed against real per-endpoint costs (live-tested) and real code logic (read directly), but not yet against a real production-scale generation run — treat "the tier likely carries KR1" as the current best evidence, not a guarantee, until that's confirmed. The tier-upgrade trigger (Section 7.4 / OQ6) remains the safety net if real-world reconciliation-trigger rates or cache-hit rates turn out worse than this analysis assumes.
...
- User manually inputs ingredients currently at home with rough quantities, from Step 2 onward (7.1) or via the conversational assistant (F11) at any time
- **MVP:** entered items are passed to F3 as a soft preference (`includeIngredients`) so generation is biased toward using them, and they're excluded from the F4 grocery list
