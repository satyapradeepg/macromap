...
User confirms or adjusts their weekly macro targets and optional grocery budget. This becomes the constraint the meal plan is built around. Held in guest session until account creation; saved to profile and pre-filled on all future weeks once an account exists.
...
- Spoonacular results cached server-side to minimise API calls; repeated plan generations reuse cached recipes where constraints haven't changed. Cache also serves as the fallback source when the API is unavailable
...
- Ingredients deduplicated and quantities summed across all recipes
...
|-------|-------------------|-----------------------|
| Recipe + nutrition | **Spoonacular API** (paid from day one — free tier is development-only, not for commercial use) | Single API call returns recipe + per-ingredient nutrition data. `/recipes/complexSearch` accepts macro targets and dietary filters directly. Paid tier starts at $29/month (1,500 points/day; ~71 full plan generations/day is a **ceiling** assuming 1 query/meal — cascade fallback, OQ2, can add up to 3× queries per meal on tight constraints, so real sustainable capacity may be meaningfully lower until measured). Revenue break-even: 4 Pro subscribers. Evaluate Edamam as alternative if pricing is a blocker. |
...
**Known cost constraint:** Spoonacular paid tier ($29/month) is a deliberate operating cost from day one — revenue break-even is 4 Pro subscribers ($36/month). Monitor API point consumption; 1,500 points/day supports up to ~71 full plan generations at 1 query/meal, but treat this as a ceiling, not a target — cascade retries reduce it in practice. Cache results aggressively to stay within quota during early growth, and track the actual cascade-trigger rate post-launch to recompute real daily capacity.
...
| ~~OQ4~~ | ~~Multi-serving recipe scaling?~~ | Engineering | **Resolved: Auto-scale using Spoonacular's `servings` field. Formula: `grocery quantity = (recipe ingredient amount ÷ recipe servings) × times meal appears in week`. Spoonacular already returns macro data per serving — no custom nutrition scaling needed. Ingredient quantities from `extendedIngredients` are divided by `servings` to get per-meal amounts, then summed across the full plan for the grocery list.** |
...
| OQ6 | Register Spoonacular paid API key and confirm point consumption per endpoint before engineering starts. Also measure actual cascade-trigger rate in early testing to validate real daily plan-generation capacity (71/day is a best-case ceiling, not a guarantee). | Engineering | **Yes — immediate action required** |
