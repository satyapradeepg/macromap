...
User confirms or adjusts their weekly macro targets, optional grocery budget, and zip code (used for regional price estimates — see F2, F4). This becomes the constraint the meal plan is built around. Saved to `profiles` and pre-filled on all future weeks. (Originally "held in guest session until account creation" — superseded, see the Step 1/Step 2 correction notes: there's no guest session left to hold it in.)
...
On day 7 (or whenever the user is ready), a "Plan next week" prompt appears. App pre-fills the same macro targets and budget from the user's saved profile (Step 2 baseline). Any edits made in this step are saved back to the profile and become the new baseline for future weeks — there is no session-only state. User can adjust, then generate a new plan. Previous week's plan is archived and viewable.
...
| **E3 — Grocery Management** | F4 | MVP | Deduped shopping list, price estimates, manual overrides |
...
- **Known open gap, now confirmed HIGH-severity (OQ9):** this weekly total is never reconciled against F2's `weekly_budget_usd` or F3's budget-aware filtering (which checks a different, recipe-level Spoonacular estimate during generation) — the two numbers structurally never talk to each other. Live-tested across 4 real profiles spanning $35–$275 stated weekly budget (2026-07-25): all four landed in the same $78–90/week real grocery total, i.e. the stated budget currently has no measurable effect on the real number, with no UI warning when this happens. See OQ9 — still an open product decision, nothing built yet.
...
| OQ9 | **New, surfaced while building/verifying F4's grocery pricing, July 2026:** F2's `weekly_budget_usd` and F3's budget-aware filtering check a recipe-level Spoonacular estimate (`pricePerServingCents`) during generation, but F4's real grocery-list total is priced independently (Spoonacular ingredient `estimatedCost`, Tavily fallback) — **the two numbers are never compared anywhere in the code**, confirmed by a direct cross-file trace (`ranking.ts`/`orchestrate.ts` for the budget check, `groceryData.ts`/`aggregate.ts` for the grocery total). **Live-tested and confirmed (2026-07-25):** 4 real profiles spanning $35–$275 stated weekly budget (plus one with no budget) all landed in the same $78–90/week real grocery total — the stated budget currently has no measurable effect on the real number, and there's no UI warning when this happens. This is no longer a theoretical gap; it's a confirmed HIGH-severity finding directly undercutting "budget-aware planning," the Pro tier's main differentiator (Section 06). **Still open: which fix to build** — options already scoped (surface the real total against the stated budget on F4 with an over/under indicator; bias generation toward real ingredient cost, not just the recipe-level heuristic; react after the fact; or relabel the budget field to set correct expectations) — nothing built yet, needs a product-direction decision. | Design + Engineering | **No — F4 already ships useful value without this; resolve before leaning on "budget-aware" as a marketed guarantee, and before the next round of Pro-tier messaging** |
