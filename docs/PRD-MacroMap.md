...
- Pantry contents, if entered (biases recipe selection toward on-hand ingredients — see F3, F6). No longer V2-gated.
...
User can swap individual meals they don't like, or ask the conversational assistant (F11) to adjust a meal, the pantry, or a constraint in plain language at any point in Steps 2–5.
...
| **E2 — Meal Planning** | F3, F6, F11 | MVP | AI-generated weekly plan, pantry-aware and macro-matched recipes, conversational plan assistant |
| **E3 — Grocery Management** | F4 | MVP | Deduped shopping list, manual overrides |
| **E6 — Discovery & Integrations** | F10 | MVP | Calendar export |

**MVP critical path:** E1 → E2 → E3 (in that order — each epic depends on the previous)
**Rescoped from the original plan:** F6 (pantry log — manual entry, generation-time use, grocery-list exclusion) moved into E2/MVP so pantry can bias recipe selection from day one, per the pantry-first architecture change.
...
- **Swap is now pantry-quantity-aware too (built July 25 2026):** before ranking a replacement, the swap accounts for what every *other* slot in the current plan has already consumed from pantry, so it doesn't recommend a candidate as if the full pantry stock were untouched by the rest of the week's plan. Deliberately stays on cheap word-boundary/id matching only — it does **not** run the LLM identity-match or unit-conversion resolution described in F4/F6 below, to avoid adding cold-cache latency to what's meant to be an instant single click. Full parity with the post-generation critique-repair pass (which does use the resolved, LLM-matched tracker) is a documented, deliberately deferred option — revisit only if usage data shows fuzzy-name/cross-unit pantry mismatches are common enough in swaps specifically to justify the latency tradeoff.
