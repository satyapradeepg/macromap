...
2. **AI can reason across data sources.** The Model Context Protocol (MCP) lets an AI agent pull from recipes, nutrition data, and personal pantry information simultaneously — and make decisions that require all three at once. That cross-source reasoning is the core of MacroMap.

### Why does the problem go unsolved today?
Existing tools solve one piece each. MyFitnessPal tracks macros. Mealime suggests recipes. Cronometer measures nutrients. Prospre gets furthest of any competitor identified so far — it pairs USDA-grounded macro-fit meal generation with dietary/dislike filtering and grocery lists in a single app — but has no pantry awareness (a deliberate omission per its own team). None connect macro precision, meal planning, and personal pantry awareness into a single planning flow.
...
- **Strategic fit:** AI-native product in a space where incumbents are traditional CRUD apps. First mover advantage on contextual reasoning across nutrition, recipes, and pantry.
...
**Their job:** "Plan a week of meals that hits my protein target — without spending my Sunday doing it manually."

**Constraint:** They have tried existing apps and found them incomplete. They won't tolerate inaccurate macro data.

**Example personas:** Marcus (bulking, immediate Pro buyer) and Priya (cutting, will pay if the first week saves her meaningful time).
...
Competitors solve one dimension — Prospre solves two (macro-fit generation + meal planning) but stops short of pantry awareness. MacroMap solves the intersection:

> "Make this chicken stir-fry — you already have soy sauce, and it hits your protein target."

No competitor today can make that recommendation — it requires macro precision, meal planning, and pantry awareness together, and MacroMap is the only one holding all three data points simultaneously.
...
```
Onboarding → Set Weekly Goal → Generate Meal Plan → View Grocery List → (repeat weekly)
```
...
App calculates TDEE using the Mifflin-St Jeor formula and suggests macro targets. User can accept or manually override. **Superseded (migration `0034`, the Auth0 identity swap):** the guest/no-account flow described here and in Step 2 below was replaced — every page, including onboarding, now sits behind a real Auth0 login (`src/proxy.ts`'s `auth0.middleware()`); there is no anonymous/guest session anymore. See the corrected note after Step 2 and Section 7.4's Application Architecture row.

**Step 2 — Set Weekly Goal**
User confirms or adjusts their weekly macro targets. This becomes the constraint the meal plan is built around. Saved to `profiles` and pre-filled on all future weeks. (Originally "held in guest session until account creation" — superseded, see the Step 1/Step 2 correction notes: there's no guest session left to hold it in.)
- Weekly calorie target = daily TDEE × 7 (calculated in Step 1, surfaced here as a weekly number for clarity)
- **Pantry (optional):** user can list ingredients currently on hand with rough quantities, moved up from V2 into this step (see F5). If provided, Step 3's generation is biased toward recipes that use them and excludes them from the Step 4 grocery list; if skipped, generation proceeds exactly as before. Editable at any point via the conversational assistant (F7), not just here.

**Account creation — original decision, since superseded:** this section originally called for guests to complete the full loop (onboarding → plan → grocery list, Steps 1–4) with no account, with the signup wall appearing once the user needed to save progress or return across sessions, on the reasoning that this removes friction before the user has seen the product's value (KR4). **Superseded by migration `0034` (the Auth0 identity swap):** Supabase anonymous auth was removed entirely, and Auth0 now gates every route, including onboarding, from the very first page. There is no guest state and no deferred signup wall — an account is required before Step 1 can even be started. The KR4 friction-reduction goal this decision was chasing is no longer addressed by the onboarding flow itself; if onboarding-completion data shows this matters, it would need a different mitigation (e.g. a lighter-weight signup, or a pre-signup preview of the product) rather than the guest-session approach described above.
...
- Pantry contents, if entered (biases recipe selection toward on-hand ingredients — see F3, F5). No longer V2-gated.
...
User can swap individual meals they don't like, or ask the conversational assistant (F7) to adjust a meal, the pantry, or a constraint in plain language at any point in Steps 2–5.

**Step 4 — Grocery List**
App generates a deduped shopping list from the meal plan — consolidating quantities across all recipes (e.g. if 4 meals need chicken breast, one line item shows the total quantity needed).

**Step 5 — Weekly Cycle (every week)**
On day 7 (or whenever the user is ready), a "Plan next week" prompt appears. App pre-fills the same macro targets from the user's saved profile (Step 2 baseline). Any edits made in this step are saved back to the profile and become the new baseline for future weeks — there is no session-only state. User can adjust, then generate a new plan. Previous week's plan is archived and viewable.

---

### 7.2 Epics

Features are grouped into four epics. Each epic maps to a coherent unit of user value that can be planned and shipped independently.
...
| **E2 — Meal Planning** | F3, F5, F7 | MVP | AI-generated weekly plan, pantry-aware and macro-matched recipes, conversational plan assistant |
| **E3 — Grocery Management** | F4 | MVP | Deduped shopping list, manual overrides |
| **E4 — Discovery & Integrations** | F6 | MVP | Calendar export |

**MVP critical path:** E1 → E2 → E3 (in that order — each epic depends on the previous)
**Rescoped from the original plan:** F5 (pantry log — manual entry, generation-time use, grocery-list exclusion) moved into E2/MVP so pantry can bias recipe selection from day one, per the pantry-first architecture change.
...
- **Superseded:** this bullet originally described a guest session (no account yet, onboarding inputs persisted in browser localStorage, account only required once the user needed to save progress across sessions). That guest flow no longer exists — see Section 7.1 Step 1's correction note. An account is required before onboarding starts, and inputs are written straight to `profiles`, not localStorage.
...
- 3 meals/day, 7 days, macro-matched, allergy/dietary filtered
...
- **Pantry-aware querying (moved up from V2, see F5):** if the user has entered pantry items, they're passed to the Spoonacular query as a soft preference (`includeIngredients`) alongside the existing hard filters, so candidates that use on-hand ingredients rank higher; this never overrides macro/dietary/allergen constraints, it only biases which matching recipe is preferred.
- **Candidate selection when multiple recipes match a tier:** deterministic, not AI-judged. Score = weighted deviation from the per-meal target — `|protein_actual − protein_target| / protein_target × 2 + |calories_actual − calories_target| / calories_target` (protein weighted 2× per the macro-split priority in F1), with a deduction for pantry-ingredient overlap — lowest score wins. Ties broken by highest Spoonacular `aggregateLikes`. Variety ("not used elsewhere this week") is enforced separately, in the cross-slot claim-resolution pass (OQ7) — not as a ranking tie-break here, since all 21 slots resolve concurrently and no slot knows what others picked at ranking time. See Agent 2 in ai-agents.md.
- **Pantry overlap is now quantity-aware, not a static yes/no signal (built July 25 2026, `web/src/lib/mealplan/pantryRemaining.ts`):** a live pantry "remaining quantity" tracker depletes as slots get claimed during generation, so a recipe's pantry credit shrinks as other slots use up the same on-hand stock — replacing the old pure boolean "does this recipe touch a pantry ingredient, yes/no" deduction above. **Known, accepted limitation:** the very first parallel-scored pass across all 21 slots is ranked from one shared snapshot before any slot is claimed, so it can't yet reflect depletion from slots claimed in that same pass — only slots re-scored afterward (exhaustion retries, weekly reconciliation, protein-floor repair, post-generation critique repair, and the standalone "swap meal" action below) see genuine, shrinking pantry credit. A future single-pass restructure to close this gap is scoped but deliberately not built, pending evidence the limitation produces a materially worse recommendation in practice.
...
- **Weekly reconciliation pass:** per-meal tolerance (±10–30%) does not guarantee the weekly aggregate lands on target, so after all 21 meals are selected, sum actual protein/calories/carbs/fat and compare against a tighter ±5% weekly band. If outside band, first try attaching a snack/add-on (above) to the meal(s) furthest from target; only if that can't close a large enough gap does the Orchestrator re-query up to 3 meals with the most slack, with bounds nudged toward the deficit. Capped at 3 extra queries per plan to protect API quota (see Section 7.4 capacity note). If still outside ±5% after the cap, the plan is generated as-is rather than the system claiming an exact match it didn't hit.
- User can tap "swap meal" on any individual meal to get an alternative (triggers a new Spoonacular query with the same constraints, excluding the rejected recipe), or ask the conversational assistant (F7) to do the same in plain language. If cascade fallback and the AI composition fallback both fail to find a replacement for that single slot, show the same "no match — here's the blocking constraint" prompt used at OQ2's final tier, scoped to just that meal — never a dead end
- **Swap is now pantry-quantity-aware too (built July 25 2026):** before ranking a replacement, the swap accounts for what every *other* slot in the current plan has already consumed from pantry, so it doesn't recommend a candidate as if the full pantry stock were untouched by the rest of the week's plan. Deliberately stays on cheap word-boundary/id matching only — it does **not** run the LLM identity-match or unit-conversion resolution described in F4/F5 below, to avoid adding cold-cache latency to what's meant to be an instant single click. Full parity with the post-generation critique-repair pass (which does use the resolved, LLM-matched tracker) is a documented, deliberately deferred option — revisit only if usage data shows fuzzy-name/cross-unit pantry mismatches are common enough in swaps specifically to justify the latency tradeoff.
...
- **Quantity scaling (built + corrected July 2026):** each recipe's ingredient amounts are scaled to reflect only the portion actually planned for that one meal — the same per-slot macro-fit factor applied everywhere else, divided by the recipe's native serving count (Spoonacular's ingredient amounts are for the whole recipe batch, e.g. "makes 4 servings," not one serving). Live-tested: this was the dominant fix for a real, badly inflated grocery-quantity bug found in `rankCandidates`.
- **Unit reconciliation:** convert matched entries to a common unit using the `measures.metric.amount`/`unitShort` already returned in the same complexSearch call (see F3's `addRecipeInformation`/`fillIngredients` note) — sum locally, no extra API call. If `metric.unitShort` still doesn't match across matched entries for the same `id` (or `measures` is missing), don't force a merge — list as a separate line under that ingredient with a "combine manually" prompt. Volume-based measures (e.g. "chopped," "cups") converted to weight are Spoonacular's own density estimate, not exact.
- Ingredients deduplicated and quantities summed across all recipes using the key and unit rules above
- **Pantry subtraction (built + substantially reworked July 2026):** a matching pantry item's structured quantity forms a pool that's drawn down across *every* matching grocery line (not reapplied in full per line — fixed a real double/triple-counting bug), and a line only hard-excludes when **none** of its matching pantry items have a usable/convertible quantity, not just when one of several matches lacks one. Matching itself is now an **LLM identity classifier** (`web/src/lib/grocery/identityMatch.ts`, Claude Haiku tier, forced tool-call, cached globally per ingredient-name pair — migration `0019`) that judges name-level identity rather than Spoonacular id or plain substring matching, since one real ingredient (e.g. garlic) routinely resolves to several different ids across a plan's recipes. Cross-category unit mismatches (e.g. a pantry entry in ml offsetting a recipe line in grams) are resolved via a real, ingredient-density-aware Spoonacular conversion call (`web/src/lib/grocery/unitConversion.ts`, `/recipes/convert`, cached globally — migration `0020`) rather than being hard-excluded — live-confirmed density-accurate (500ml olive oil → 456.5g, matching real olive oil density). See F5.
- One-tap copy to clipboard

#### F5 — Pantry Log (P1) · E2 Meal Planning — moved up from V2
- User manually inputs ingredients currently at home with a rough free-text quantity note, from Step 2 onward (7.1) or via the conversational assistant (F7) at any time
...
- **MVP:** entered items are passed to F3 as a soft preference (`includeIngredients`) so generation is biased toward using them, and now also feed a quantity-aware pantry-depletion tracker inside F3's own ranking (see F3's pantry-overlap note above) and the standalone swap action — not just a query-time bias. In F4's grocery list: a matching entry's structured quantity draws down a pool across every matching line (see F4's reworked pantry-subtraction bullet above), with matching now judged by an LLM identity classifier and cross-unit conversion resolved via a real Spoonacular density-conversion call, not deterministic same-unit arithmetic alone; a line only falls back to full exclusion when no matching pantry item has a usable/convertible quantity, not on the first unresolvable match — a meaningfully narrower gap than the original all-or-nothing design
- **V2:** automatic 7-day expiry unless refreshed
- Fully optional — skipping pantry entry leaves generation and the grocery list unchanged from the pre-pantry design

#### F6 — Calendar Export (P2) · E4 Discovery & Integrations
- One-tap "Export to Calendar" button on the weekly meal plan view
- Generates a `.ics` file (iCalendar/RFC 5545 format) via a hand-built generator — no external calendar library
...
#### F7 — Conversational Plan Assistant (P1) · E2 Meal Planning — new
- Persistent chat available across Steps 2–5 (Set Weekly Goal through Weekly Cycle) — not a one-shot Q&A widget
- Understands plain-language requests to: edit pantry contents, swap a specific meal, or change a dietary constraint/allergy/dislike
...
MacroMap is built as an AI agent orchestrating multiple MCP data sources. Each layer is independently swappable as the product scales.

| Layer | MVP Implementation | Notes / Scale-up Path |
|-------|-------------------|-----------------------|
| Recipe + nutrition | **Spoonacular API** (paid from day one — free tier is development-only, not for commercial use). **Re-scoped, then re-scoped again (July 2026):** an initial recompute suggested this tier might carry KR1's full 1,000-subscriber target after all — since superseded by a real 15-profile live batch (see OQ6's "Third update"); real measured cost is materially higher, so this conclusion no longer holds. | Single API call returns recipe + per-ingredient nutrition data. `/recipes/complexSearch` accepts macro targets and dietary filters directly. Paid tier starts at $29/month (1,500 points/day). **Ceiling, current as of the 2026-07-22 live batch (OQ6):** real measured cost averages **~34.2 pts/generation** (not the `2.00 + 0.06 × resultsReturned`/"1-2 calls per plan" formula this row previously cited — that model undercounted real call volume once meal types stopped sharing one cache key and the post-generation critic/repair pass was added). Real capacity: **~44 plans/day (~306/week) — about half the ~600/week KR2 needs, not comfortably above it.** Not yet root-caused in full; Edamam ($38/month) is the validated fallback vendor if this becomes a blocker. Revenue break-even at this tier: 4 Pro subscribers — recompute once the tier changes. **Tier-upgrade trigger:** move to a higher Spoonacular tier or evaluate Edamam once measured daily generation volume reaches ~75% of this corrected ceiling. |
| Pantry & constraints | Custom MCP (user-authored). Pantry contents read at plan-generation time from MVP (F5, moved up from V2), not just used to filter the grocery list — as of July 25 2026, quantity-aware: a live depletion tracker (`pantryRemaining.ts`) reduces a recipe's pantry credit as other slots use up the same stock, both during generation and in the standalone swap action. Grocery-list matching (F4) uses an LLM identity classifier (Claude Haiku, cached globally) for name-level matching, and a real Spoonacular density-conversion call (cached globally) for cross-unit-category matches — not deterministic same-unit arithmetic alone. | Automatic 7-day expiry remains V2. Synced receipt scanning (Post-MVP). |
| Calendar export | `.ics` file via a hand-built generator, no external library (no API, no OAuth) | MVP. Upgrade to Google Calendar API direct sync (OAuth, 1M req/day free) post-MVP. |

**Known cost constraint & staged capacity plan — current conclusion (2026-07-22 live batch, see OQ6):** Spoonacular's $29/month paid tier was originally scoped purely for development and an early/small production cohort, on the assumption it couldn't carry KR1's full 1,000-subscriber target. A code-grounded recompute briefly suggested otherwise (~134-268 plans/day) — that estimate is now known stale: a real 15-profile live batch measured actual cost averaging **~34.2 pts/generation**, putting real capacity at **~44 plans/day (~306/week) — about half the ~600/week KR2 needs, not comfortably above it.** Risk re-upgraded to Medium (Section 6/product-brief.md). Revenue break-even at this tier is still 4 Pro subscribers ($36/month) — that's a cost question, unaffected by the capacity finding. Cache results aggressively (see the two-cache split in F3/ai-agents.md Agent 2) — real-world cross-user cache hits could push effective capacity higher, but this hasn't been measured. Edamam ($38/month) remains the validated fallback vendor. **Upgrade trigger:** track measured daily generation volume against this corrected ceiling; at ~75% of ceiling, move to a higher Spoonacular tier or evaluate Edamam — decide proactively, before capacity is actually exhausted, since an outage during a growth spike is the worst possible time to discover the ceiling.

**Application architecture (resolved, then partially superseded — see auth note below):** Next.js (App Router, TypeScript) deployed on Vercel (Azure Container Apps for the current class-hosted deployment) — one codebase serves both the frontend and the API routes/server actions that call Spoonacular, so those keys are read from server-side environment variables and never reach the client bundle. Supabase (Postgres) provides the database: user profiles, pantry/constraints, chat history, and both Agent 2 caches (the cross-user query-result cache and the per-user last-successful-plan cache — see ai-agents.md) live as Postgres tables; no separate cache service (e.g. Redis) is introduced at MVP scale, only if measured query volume later demands it. **Auth, superseded (migration `0034`):** Supabase's built-in anonymous auth originally implemented the guest→account flow described in Section 7.1 Step 1/2 (a guest session starting as an anonymous Supabase user, converted to a permanent account once the user needed to save progress across sessions). That mechanism was removed entirely — **Auth0 is now the sole identity provider**, gating every route (`src/proxy.ts`) from the first page onward, with no anonymous/guest tier. Every user-owned table now keys on `user_id text` (an Auth0 subject id, e.g. `auth0|...`) with RLS policies of the form `(auth.jwt() ->> 'sub') = user_id`, not the `uuid`/`auth.uid()` pattern this row originally implied. Net new infrastructure cost pre-revenue: $0 beyond Spoonacular's $29/month (both Vercel/hosting and Supabase free tiers cover a pre-seed cohort); Auth0's free tier covers this stage too.
...
|---|-----------|--------------|-----------------|
| A1 | Users will trust AI-generated macro calculations enough to follow the meal plan | High — if users don't trust the numbers, retention collapses. Elevated further by A5: some meals are now AI-composed/edited, not pure Spoonacular lookups. | 5–10 user interviews before launch |
...
| A3 | Users will log their pantry manually (without photo/receipt scanning) | Medium — if friction is too high, the pantry feature won't be used and F3's pantry-aware generation loses its input | Measure pantry log completion rate in first 2 weeks of MVP (see H5) |
| A4 | $9/month is below the willingness-to-pay threshold for Pro users | Medium — early signal positive (friends and forum outreach show interest in paying), but informal interest ≠ actual conversion. | Run 5–10 structured interviews to confirm price point before launch |
| A5 | AI-composed/edited recipes and snack add-ons — grounded in Spoonacular's per-ingredient nutrition data, never LLM-estimated — are macro-accurate and feel realistic enough that users don't rate them as lower quality than pure Spoonacular recipes | High — same trust surface as A1, extended to a new mechanism introduced by the pantry-first/AI-composition architecture change | Tag every meal by source (`spoonacular`/`ai-composed`/`ai-edited`/`snack-addon`) and compare acceptance/swap rates — see H6 |
...
- Grocery list (F4)
- Pantry log — manual entry, generation-time use, grocery-list exclusion (F5, moved up from V2)
- Conversational plan assistant — chat-driven pantry/meal/constraint edits (F7, new)
...
**Does not include:**
- Pantry item auto-expiry (7 days) — added in V2
