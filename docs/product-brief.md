...
| **Pantry MCP** | Custom-authored MCP: pantry inventory, dietary constraints, dislikes, allergies, weekly budget. The personalization layer — now read *before* meal generation from day one (moved up from V2), biasing recipe selection toward what's on hand. Grocery-list handling (built July 2026): a pantry entry with a structured quantity (amount + unit, optional alongside the original free-text note) has its amount deterministically subtracted from the matching grocery line; otherwise the ingredient is excluded entirely, the original all-or-nothing behavior. |
...
**Pro includes:** Budget-aware meal planning, per-ingredient price estimates (Spoonacular primary, Tavily fallback — see Price MCP above), pantry cloud sync across devices (V2 — local-only pantry storage is Free/MVP), full nutrition dashboard with weekly analytics, shopping list export.
...
| **Medium (new, found July 2026)** | F4's real grocery total is never checked against F2/F3's stated budget | The two numbers come from independent mechanisms — F3's budget-aware filtering checks a recipe-level Spoonacular estimate during generation, F4 prices real ingredients separately — and nothing in the code compares them (see PRD OQ9). A user could set a budget, get a "budget compliant" plan, and see a grocery total meaningfully over or under it with no indication. Not yet tested across multiple real budgeted profiles — first item on next session's list. |
...
- Pantry log — manual entry, generation-time biasing, grocery-list exclusion or quantitative reduction (moved up from V2)
